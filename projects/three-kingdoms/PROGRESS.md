...
좌표판 D5·D6 검토       회신 04       완료 — ㉳(D6) 채택 «권고»
앵커 제작              —             미착수. Q1 대기
```
...
| 세력색 런타임 버그 | `alive:false` 잔류가 원인. `Events.clanColor(st)` 로 해결 | `brief/IDENTIFICATION_SYSTEM.md` §1-2 |
...
| 3 | 원화 보관 (git-lfs) | **나머지와 독립.** 먼저 답이 와도 좋다 |
...
### 다음 세션 과제 (Studio 몫 · 아직 확인 못 함)

- PixelLab · Retro Diffusion 의 **현재 API 제공 여부 · 요금 · 출력 규격**
- `2d-assets` 저장소 실제 상태 재확인 — 부대 스프라이트 착수 시점에
- **Galmuri 도트 글꼴의 실제 표시 크기** — 아이콘 선 두께를 획 두께에 맞춰야 한 벌로 보인다. 앵커 제작 전
- claude.ai 커넥터(Drive 등)가 **미인증**이다. 원본이 Drive 에 있으면 지금은 가져올 수 없다
...
| 후보 | 현재 위치 | 승격 시점 |
|---|---|---|
| `lib/providers.js` (bfl·replicate·openai·dryrun) | 게임 저장소 | **승격 조건은 이미 충족**(tteoklak 이 같은 필요를 따로 만났다). 디렉터 승인 뒤 별도 작업으로 |
