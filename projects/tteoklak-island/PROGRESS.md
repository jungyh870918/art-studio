...
- 스냅샷 시점: **2026-08-17 22:10** (게임 저장소 `2b1e3f8` 까지 대조)
...
게임 쪽 문서 세로 정합     게임 `2b1e3f8`     완료 — 옛 가로 수치 일소 · 규격 세 건 회수
```

**막고 있는 것은 판단이 아니라 손이다** — 초상 1단계 프롬프트 5블록이 조립까지 끝나 있고,
**이 세션에 이미지 생성 수단이 없어서** 사람이 도구에 붙여야 다음 칸으로 간다.

**게임 쪽이 21:25 에 문서를 코드 실측에 맞추면서 세 가지가 Studio 로 되돌아왔다** —
인물 표시 크기 · 인벤 아이콘 칸 · 저장 썸네일. 셋 다 §3 에 넣었다.
...
| 배경 규격 | **1024×1536 (2:3) 유지.** 게임이 제안한 1418 은 산수는 맞았지만 **여유가 0이 된다** — 위아래 59px 씩 잘리는 그 잘림이 설계다 | `STUDIO_VERDICT` §2-B · 게임 `docs/art/README.md` |
...
| **하단 Modern 셸** | **이번 범위가 아니다 — 「다음 단계」.** 그림 교체가 아니라 상호작용 모델 변경이고, **아이콘 규격이 셸에서 나오므로** 셸 없이 뽑으면 안 쓰는 그림이 된다 | 게임 회신 · `GAME_RESPONSE` §4 |
| 그래서 **발주 금지** | 행동 아이콘 · 과녁 · 진행 트랙 · FAB 가방 · **P2 배틀 반신** · 논거 카드 11장 | 같은 곳 · `ASSET_MANIFEST` §5 |
...
| | 무엇 | 언제 닫히나 | 지금 막나 |
|---|---|---|---|
| **인물 표시 크기** | 한개미가 밴드의 몇 %인가 | **재는 것은 Studio 몫, 정하는 것은 사람 몫.** 게임이 잴 자를 지정했다 — `05_subway_night` 에 그려진 승객 키 | **D3 · D4 를 막는다** |
| **D3 · D4** | 캐릭터 제작 방식과 프레임 수 | 위 기준이 정해진 뒤. **B(프레임 생성)는 이미 사실상 탈락** — 남은 것은 A(컷아웃)·E(3D blockout) | 스프라이트 145프레임을 막는다 |
...
**게임 쪽이 21:25 에 되돌려 준 규격 세 건** — 셋 다 옛 가로 화면(320×180) 시절 값이 남아 있던 것이다.

| 게임 QA # | 무엇 | Studio 가 할 것 |
|---|---|---|
| 18 | `Actor` 가 32×48 을 배율 없이 그린다 = **밴드의 3.7%** | 위 「인물 표시 크기」. **가장 크다** |
| 19 | 저장 썸네일이 프레임 위 135px 만 자른다 | 아트 발주가 아니다. 게임 쪽 수정 |
| 20 | 인벤 아이콘 칸이 **16×12** 라 정사각이 눌린다. 게임은 규격을 **48×36** 으로 적어 두었다 | **칸이 정사각이 아닌 것 자체를 먼저 볼 것** — 아이콘 12개를 48×36 으로 발주하면 목업의 정사각 아이콘 언어와 어긋난다 |
...
1. **초상 1단계 생성** — [`brief/PROMPTS_PORTRAITS.md`](brief/PROMPTS_PORTRAITS.md) 의 5블록을
   **그대로 복사해** 붙인다. 손으로 고쳐 쓰지 않는다 (R3)
   ...
   - 받은 것은 `candidates/portraits/<id>/` 에 넣는다. **다섯 폴더 다 만들어져 있다**
     (`han_gaemi` · `sera` · `boss` · `park` · `broker`. `sera` 는 목업 채택 뒤 뒤늦게 생겼다)
2. **1단계는 얼굴만 정한다.** 표정은 고른 얼굴을 앵커로 2단계에서 파생시킨다 —
   표정마다 따로 4장씩 받으면 같은 사람으로 안 보이는 문제가 재발한다
3. 배경 나머지 4장 + 기존 2장 재수령
4. **인물 기준 산출** — `candidates/backgrounds/05_subway_night.png` 의 「조는 승객」 키를
   밴드 비율로 환산해 한개미의 목표 높이를 낸다. **Studio 가 재서 안을 올리고 사람이 정한다** →
   그 뒤 D3 · D4
